// RUN: %clangxx -fsycl -std=c++17 %s -o %t.out
// RUN: env SYCL_DEVICE_TYPE=HOST %t.out
// RUN: %CPU_RUN_PLACEHOLDER %t.out
// RUN: %GPU_RUN_PLACEHOLDER %t.out
// RUN: %ACC_RUN_PLACEHOLDER %t.out
//==---------accessor_simplification.cpp - SYCL accessor basic test --------==//
//
...
int main() {
  // Host accessor.
  {
    ...
    sycl::read_accessor acc_src(buf_src);
    //auto acc_dst = buf_dst.get_access<sycl::access::mode::read_write>();
    sycl::accessor acc_dst(buf_dst);
  ...
  // Discard write accessor.
  {
    try {
      ...
      sycl::read_accessor host_acc(buf);
  ...
  // Discard read-write accessor.
  {
    try {
      ...
      sycl::discard_read_write_accessor host_acc(buf);
    } catch (cl::sycl::exception e) {
      std::cout << "SYCL exception caught: " << e.what();
      return 1;
    }
      ...
      int data = -1;
      {
        ...
        queue.submit([&](sycl::handler &cgh) {
          sycl::accessor<int, 0, sycl::access::mode::read_write,
              ...
              B(b, cgh); // is not being simplified due to not deducable dimension
...

  {
    ...
    sycl::accessor<int, 0, sycl::access::mode::read,
        ...
        acc4(buf1); // is not being simplified due to not deducable dimension
    //sycl::accessor<int, 1, sycl::access::mode::read,
    //               sycl::access::target::host_buffer>
    //    acc5(buf2);
    sycl::read_accessor acc5(buf2);
    //sycl::accessor<int, 1, sycl::access::mode::read,
    //               sycl::access::target::host_buffer>
    //    acc6(buf3, sycl::range<1>(1));
    sycl::read_accessor acc6(buf3, sycl::range<1>(1));
