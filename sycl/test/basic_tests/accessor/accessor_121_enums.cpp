...
int main() {
  // Check that SYCL 1.2.1-defined accessor enums compile
  ...
  Q.submit([&](sycl::handler &h) {
    // Test access::mode enums
    {
      ...
      sycl::accessor<int, 1, sycl::access::mode::discard_read_write,
                     sycl::access::target::global_buffer>
          a3(B, h);
    ...
    static_assert(
        std::is_same<
            decltype(sycl::accessor<int, 1, sycl::access::mode::write>{B, h}),
            decltype(sycl::accessor<int, 1, sycl::access::mode::write,
                                    sycl::access::target::global_buffer>{
                B, h})>::value,
